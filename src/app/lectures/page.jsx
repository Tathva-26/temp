...
// Mock lecture images
const MOCK_LECTURE_IMAGES = [
  "/images/lecture.jpg",
  "/images/Graph_Mining.png",
  "/images/Quantum_Computing.jpg",
];

const DEFAULT_MOCK_LECTURES = [
  {
    id: 1,
    heading: "Genetic Algorithms & Evolutionary Computing",
    description: "Explore search heuristics and optimization inspired by natural selection and genetics.",
  ...
  },
  {
    ...
    description: "Deep dive into multi-layer architectures, backpropagation, and state-of-the-art vision models.",
    picture: "/images/lecture.jpg",
    isFull: false,
    price: 0,
  ...
  },
  {
    ...
    description: "Understanding generator-discriminator dynamics, latent spaces, and synthesizing high-fidelity imagery.",
    picture: "/images/lecture.jpg",
    isFull: false,
    price: 0,
  ...
  },
  {
    ...
    description: "Analyzing large-scale interconnected data structures, node embeddings, and topological dynamics.",
  ...
  },
  {
    ...
    description: "From qubits and superposition to quantum entanglement and modern computational speedups.",
  ...
  {
    id: 6,
    heading: "Lecture Title 2",
    description: "Real-time sensor fusion, SLAM, and reinforcement learning in embodied robotic systems.",
    picture: "/images/lecture.jpg",
    isFull: false,
    price: 0,
...
export default function LecturesPage() {
  ...
  const [calloutLecture, setCalloutLecture] = useState(null);
  ...
  // A single viewport-spanning layer (the `.lecture-focus-overlay` element
  ...
  const fadeOutCallout = () => {
    ...
    if (labelEl) {
      ...
      gsap.to(labelEl, {
        ...
        ease: EASE,
        overwrite: "auto",
        onComplete: () => setCalloutLecture(null),
      });
    } else {
      setCalloutLecture(null);
    }
  ...
  const startCallout = (id, slotEl, lecture) => {
    hardResetCallout();
    ...
    setCalloutSide(side);
    setCalloutLecture(lecture);

    const titleText = String(lecture.heading ?? "Untitled").toUpperCase();
    const venueName = getVenueName(lecture.venue);
    const metaText = `${formatDate(lecture.datetime)}${lecture.time ? ` · ${lecture.time}` : ""}${venueName ? ` · ${venueName}` : ""}`;
    const descText = String(lecture.description ?? "No description available");
    const priceText = lecture.price != null ? (lecture.price === 0 ? "Free" : `₹${lecture.price / 100}`) : "Free";
  ...
  const handleCardEnter = (id, e, lecture) => {
  ...
  useEffect(() => {
    ...
    if (!entranceCompleteRef.current && lectures.length > 0) {
    ...
    const fetchLectures = async () => {
      try {
        ...
        if (response.data?.events && response.data.events.length > 0) {
          setLectures(response.data.events);
        } else {
          setLectures(DEFAULT_MOCK_LECTURES);
        }
        setError(null);
      } catch (err) {
        console.error("Error fetching lectures:", err);
        // Fallback to mock lectures so the page is previewable even if backend is offline/empty
        setLectures(DEFAULT_MOCK_LECTURES);
        setError(null);
      } finally {
        setLoading(false);
  ...
  const sortedLectures = useMemo(() => {
    const searched = lectures.filter((lecture) =>
      lecture.heading?.toLowerCase().includes(searchQuery.toLowerCase())
    );
    const filtered = searched.filter((l) => !l.isFull);
    return [
      ...filtered.filter((l) => !l.picture?.trim().endsWith("-DUMMY.jpg")),
      ...filtered.filter((l) => l.picture?.trim().endsWith("-DUMMY.jpg")),
    ];
    ...
    return (
      <div className="bg-transparent min-h-screen py-16 px-4 sm:px-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-300">Loading lectures...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div
      ref={pageRef}
      className="bg-transparent min-h-screen py-4 sm:py-10 px-4 sm:px-8 lg:px-12 text-white"
    ...
    >
      ...
      <div
        ref={focusOverlayRef}
        aria-hidden="true"
        className="lecture-focus-overlay"
        style={{
          position: "absolute",
          inset: 0,
            ...
            {sortedLectures.map((lecture, index) => {
              ...
              const displayImage =
                !picture ||
                  ...
                  picture.trim() === "/images/lecture.jpg" ||
                  picture.trim() === "/images/workshops.jpg"
                  ? MOCK_LECTURE_IMAGES[index % MOCK_LECTURE_IMAGES.length]
                  : picture;

              const lectureHref = `/lectures/${id}`;
              ...
              return (
                <div
                  ...
                  onMouseEnter={(e) => handleCardEnter(id, e, lecture)}
                    ...
                    >
                      ...
                      >
                        <Link
                          href={lectureHref}
                          className="block"
                          onClick={(e) => handleCardClick(e, id, lectureHref, displayImage)}
                        >
                          <div
                            style={{
                              position: "relative",
                          ...
                          >
                            ...
                            >
                              {price != null ? (price === 0 ? "Free" : `₹${price / 100}`) : "Free"}
                            </div>
                          </div>
                        </Link>
                        ...
                        <div
                          ...
                          className="lecture-pulse-overlay"
                          aria-hidden="true"
                        />
                        ...
                        <div
                          ...
                          className="lecture-activation-overlay"
                          aria-hidden="true"
                        />
                      </div>
                    </div>
