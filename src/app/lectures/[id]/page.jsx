...
export default async function EventPage({ params}) {
  ...
  if (!backendEnabled) {
    return <BackendStatus title="Lecture details coming soon" message="Lecture registration will be available soon." />;
  }
  ...
  const getDateDisplay = () => {
    if (!event.startTime && !event.endTime) {
      ...
      const startDate = event.startTime ? formatDate(event.startTime) : formatDate(event.datetime);
      const endDate = event.endTime ? formatDate(event.endTime) : formatDate(event.datetime);
            ...
            <div className="lg:col-span-8 bg-white/90 pp-fragment backdrop-blur-lg border border-gray-100 shadow-md rounded-2xl p-6 sm:p-8 transition-transform hover:-translate-y-1 hover:shadow-lg">
              ...
              <div className="border-t border-gray-100 pt-4 sm:pt-6">
                ...
                <p className="mt-1 inter text-sm text-gray-600">
                  Ticket details are automatically taken from your profile.
