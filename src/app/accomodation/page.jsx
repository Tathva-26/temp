...
export default function AccommodationRegistration() {
  ...
  const priceDetails = useMemo(() => {
    ...
    if (formData.startDate && formData.endDate && formData.roomType) {
      ...
      const accommodationInfo = accommodationPricing.find(
        (p) => p.type === mappedRoomType,
      );
    ...
    const totalMeals = Object.values(formData.foodDetails).reduce(
      (acc, day) => acc + day.veg + day.nonVeg,
      0,
    );
  ...
  const transformDataForAPI = (data) => {
    ...
    if (payload.room === "DORM")
      payload.room = payload.gender === "MALE" ? "DORMB" : "DORMG";
  ...
  const isStep4Valid = useMemo(() => {
    return Object.values(formData.foodDetails).some(
      (day) => day.veg > 0 || day.nonVeg > 0,
    );
  ...
  const renderInitialView = () => (
    <div className="text-center p-4 sm:p-8 animate-fade-in">
      ...
      <div className="space-y-6 text-left mb-8">
        ...
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-lg mb-4">
          <p className="text-sm text-blue-800">
            <span className="font-semibold">Note:</span> Dormitory prices are
            per person. Shared room prices are per room.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {accommodationPricing.map((pkg) => (
            <div
              key={pkg.type}
              className="bg-gray-50/80 p-6 rounded-2xl border-2 border-gray-200 shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-300"
            >
      ...
      >
        <span>
          {isLoggedIn ? "Register for Accommodation" : "Login to Register"}
        </span>
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <input
            ...
            min="2025-10-23"
            max="2025-10-26"
            className="w-full px-5 py-3.5 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-gray-800 focus:ring-4 focus:ring-gray-200 transition-all text-gray-800 placeholder-gray-400 shadow-sm"
          />
        </div>
        <div className="space-y-2">
          <label className="block text-sm font-semibold text-gray-700">
          ...
          <input
            ...
            onChange={handleInputChange}
            min={formData.startDate || "2025-10-23"}
            max="2025-10-26"
            disabled={!formData.startDate}
            className="w-full px-5 py-3.5 border-2 border-gray-200 rounded-2xl focus:outline-none focus:border-gray-800 focus:ring-4 focus:ring-gray-200 transition-all text-gray-800 placeholder-gray-400 shadow-sm disabled:bg-gray-100"
          />
        </div>
      </div>
        ...
        <div className="flex gap-3">
          {["male", "female"].map((gender) => (
            <button
              ...
              className={`flex-1 py-3.5 rounded-2xl font-semibold transition-all duration-300 shadow-sm border-2 ${formData.gender === gender ? "bg-gray-800 text-white border-gray-800 scale-105" : "bg-white text-gray-700 border-gray-200 hover:border-gray-400"}`}
            >
          ...
          <div className="flex gap-3">
            {roomOptions.map((room) => (
              <button
                ...
                className={`flex-1 py-3.5 rounded-2xl font-semibold transition-all duration-300 shadow-sm border-2 ${formData.roomType === room ? "bg-gray-800 text-white border-gray-800 scale-105" : "bg-white text-gray-700 border-gray-200 hover:border-gray-400"}`}
              >
                {room}
              </button>
            ))}
      ...
      <div className="flex space-x-3 pt-2">
        <button
          onClick={() => setStep(1)}
          className="flex-1 bg-white border-2 border-gray-300 text-gray-700 py-4 rounded-2xl font-semibold flex items-center justify-center space-x-2 hover:bg-gray-50 hover:border-gray-400 hover:shadow-lg transition-all duration-300"
        >
        ...
        <button
          ...
          className="flex-1 bg-gradient-to-r from-gray-800 to-black text-white py-4 rounded-2xl font-semibold flex items-center justify-center space-x-2 disabled:from-gray-300 disabled:to-gray-400 disabled:cursor-not-allowed hover:scale-105 transition-all duration-300 shadow-lg"
        >
          <span>Continue</span>
          <ChevronRight className="w-5 h-5" />
        </button>
  ...
  const renderStep4 = () => (
    <div className="space-y-6 animate-fade-in">
      <div className="text-center space-y-3">
        ...
        <p className="text-gray-600">Add veg or non-veg meals for each day.</p>
        <p className="text-xs text-gray-500 pt-1">
          Note: Only breakfast and lunch will be provided.
        </p>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => handleFoodCountChange(date, "veg", -1)}
                    className="w-8 h-8 rounded-full bg-white border-2 border-gray-300 text-gray-700 flex items-center justify-center hover:bg-gray-100 transition-all shadow-sm"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="text-lg font-bold text-gray-800 w-8 text-center">
                    {formData.foodDetails[date].veg}
                  </span>
                  <button
                    onClick={() => handleFoodCountChange(date, "veg", 1)}
                    className="w-8 h-8 rounded-full bg-white border-2 border-gray-300 text-gray-700 flex items-center justify-center hover:bg-gray-100 transition-all shadow-sm"
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => handleFoodCountChange(date, "nonVeg", -1)}
                    className="w-8 h-8 rounded-full bg-white border-2 border-gray-300 text-gray-700 flex items-center justify-center hover:bg-gray-100 transition-all shadow-sm"
                  >
                    <Minus className="w-4 h-4" />
                  </button>
                  <span className="text-lg font-bold text-gray-800 w-8 text-center">
                    {formData.foodDetails[date].nonVeg}
                  </span>
                  <button
                    onClick={() => handleFoodCountChange(date, "nonVeg", 1)}
                    className="w-8 h-8 rounded-full bg-white border-2 border-gray-300 text-gray-700 flex items-center justify-center hover:bg-gray-100 transition-all shadow-sm"
      ...
      <div className="flex space-x-3 pt-2">
        ...
        <button
          ...
          className="flex-[0.8] bg-white border-2 border-gray-300 text-gray-700 py-4 rounded-2xl font-semibold hover:bg-gray-50 hover:border-gray-400 hover:shadow-lg transition-all duration-300"
        >
          Skip
        </button>
        <button
      ...
      <div className="space-y-4 bg-gray-50 p-6 rounded-2xl border border-gray-200">
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="font-bold text-gray-800">
              {formData.roomType} ({formData.gender})
            </span>
          </div>

          {isStep4Valid &&
            Object.entries(formData.foodDetails).map(([date, counts]) => {
        ...
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between items-center">
            <span className="text-gray-600">
              Accommodation ({priceDetails.numberOfDays} Day
              {priceDetails.numberOfDays !== 1 ? "s" : ""})
            </span>
            <span className="font-medium text-gray-800">
              ₹{priceDetails.accommodationCost.toFixed(2)}
            </span>
          </div>
          {priceDetails.foodCost > 0 && (
            <div className="flex justify-between items-center">
              <span className="text-gray-600">
                Food ({priceDetails.totalMeals} Meal
                {priceDetails.totalMeals !== 1 ? "s" : ""})
              </span>
              <span className="font-medium text-gray-800">
                ₹{priceDetails.foodCost.toFixed(2)}
              </span>
            </div>
          )}
          ...
          <div className="flex justify-between items-center text-xs">
            ...
            <span className="font-medium text-gray-600">
              + ₹{priceDetails.platformFee.toFixed(2)}
            </span>
          </div>
          <div className="flex justify-between items-center text-xs">
  ...
  const renderStep6 = () => (
    <div className="text-center p-8 animate-fade-in space-y-4">
      ...
      <p className="text-gray-600 max-w-sm mx-auto">
        Thank you! Your accommodation has been booked. You will receive a
              ...
              {progressSteps.map((pStep, index) => (
                <React.Fragment key={pStep.id}>
                  <div className="flex flex-col items-center gap-2">
                    <div
                      className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-500 shadow-lg ${step >= pStep.id ? "bg-gradient-to-br from-gray-800 to-black text-white scale-110" : "bg-white text-gray-400 border-2 border-gray-300"}`}
                    >
                      <pStep.icon className="w-5 h-5" />
                    </div>
                    <span
                      className={`text-xs font-semibold ${step >= pStep.id ? "text-gray-800" : "text-gray-400"}`}
                    >
                      {pStep.name}
                    </span>
                  </div>
                  {index < progressSteps.length - 1 && (
                    <div
                      className={`w-16 h-1 mt-[-24px] rounded-full transition-all duration-500 ${step > pStep.id ? "bg-gradient-to-r from-gray-700 to-gray-900" : "bg-gray-300"}`}
                    />
                  )}
                </React.Fragment>
              ))}
