...
const LetterGlitch = ({
  glitchColors = ['#000000ff', '#13e021ff', '#f6ececff'],
  className = '',
  glitchSpeed = 50,
  centerVignette = true,
  centerVignetteOpacity = 0.70,
  outerVignette = true,
  smooth = true,
  lightMode = false,
  backgroundColor,
  ...
  const animate = () => {
    ...
    if (
      now - lastGlitchTime.current >= glitchSpeed
    ) {
      updateLetters();
      drawLetters();
  ...
  const centerVignetteStyle = {
    ...
    pointerEvents: 'none',
    background: lightMode
      ? `radial-gradient(circle, rgba(255,255,255,${centerVignetteOpacity}) 0%, rgba(255,255,255,0) 90%)`
      : `radial-gradient(circle, rgba(0,0,0,${centerVignetteOpacity}) 0%, rgba(0,0,0,0) 90%)`
  };
