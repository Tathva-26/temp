  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        // Aggressive factor: goes from 0 (at center) to 1 (at offset 1.2)
        const blurGrayFactor = smoothstep(0, 1, Math.min(Math.abs(offset) / 1.2, 1));
        ...
        // Blur uses the original slow curve based on 'edge'
        const blur = edgeBlur * smoothstep(0.35, 1, edge);
        
        // Grayscale uses the new aggressive curve based on 'blurGrayFactor'
        const dynamicGrayscale = blurGrayFactor;
        
        const filterStr = [];
        if (blur > 0.01) filterStr.push(`blur(${blur.toFixed(2)}px)`);
        if (dynamicGrayscale > 0.01) filterStr.push(`grayscale(${Math.round(dynamicGrayscale * 100)}%)`);
